...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	//1. Нижняя граница первого диапозона должна быть равны 0
	//2. Верхняя граница каждого диапозона, кроме последнего, должна быть равна нижней следующего
	//3. Верхняя граница должна быть всегда больше нижней границы (кроме последнего диапозона)
	...
	Если Диапазоны[0].НижняяГраница <> 0 Тогда
		...
		Сообщение.Поле = "Диапозон[0].НижняяГраница";
		Сообщение.Сообщить();	
	КонецЕсли;
	...
	Для Сч = 0 По Диапазоны.Количество() - 1 Цикл
		...
		Если Сч > 0 Тогда
			Если Диапазон.НижняяГраница <> ВерхняяГраницаПредыдущегоДиапазона Тогда
				...
				Сообщение.УстановитьДанные(ЭтотОбъект);
				Сообщение.Поле = СтрШаблон("Диапозон[%1].НижняяГраница", XMLСтрока(СЧ));
				Сообщение.Сообщить();	
			КонецЕсли;
		КонецЕсли;
	...
	Если ПоследнийДиапазон.ВерхняяГраница <> 0 Тогда
		...
		Сообщение.УстановитьДанные(ЭтотОбъект);
		Сообщение.Поле = СтрШаблон("Диапазон[%1].ВерхняяГраница", XMLСтрока(Диапазоны.Количество() - 1));
		Сообщение.Сообщить();	
	КонецЕсли;
