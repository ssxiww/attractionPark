...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если БаллыКСписанию <> 0 ТОгда
		...
		Если БаллыКСписанию > Цена Тогда
			...
			Сообщение.Текст = "Списанные баллы не должны превышать цену биелета";
		...
		Доля = БаллыКСписанию / Цена * 100;
			Если Доля > МаксимальнаяДоля Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтрШаблон("ДоляСписыванияБаллов от цены больше доупстимой(%1%%)", МаксимальнаяДоля);
				Сообщение.УстановитьДанные(ЭтотОбъект);
				Сообщение.Поле = "БаллыКСписанию";
				Сообщение.Сообщить();	
			КонецЕсли;
